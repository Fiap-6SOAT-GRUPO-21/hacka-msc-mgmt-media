...
```bash
docker build -t hacka-msc-mgmt-media:latest .
```

2. Verficar se a imagem foi criada corretamente

```bash
docker images | grep hacka-msc-mgmt-media
```

3. Caso queira deletar a imagem

```bash
docker rmi hacka-msc-mgmt-media:latest
```

4. Subir a imagem no Docker Hub

```bash
docker tag hacka-msc-mgmt-media:latest josevitordurante/hacka-msc-mgmt-media:latest
docker push josevitordurante/hacka-msc-mgmt-media:latest
```
...
#### Aplicação hacka-msc-mgmt-media
